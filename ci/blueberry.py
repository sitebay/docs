...
def require_yaml(file_yaml, **kwargs):
    ...
    for header, req in requirements.items():
        if req["required"] and header not in file_yaml:
            return str(kwargs.get("filename")), f"Missing required metadata: {header}"
...
def only_allowed_yaml(file_yaml, **kwargs):
    ...
    for header in file_yaml.keys():
        if header not in requirements.keys():
            return str(kwargs.get("filename")), f"Non-allowed metadata: {header}"
        ...
        if header in file_yaml.keys():
            ...
            if type == "link":
                if not re.search(LINK_REGEX, val):
                    return filename, f"Invalid metadata format: {val}"
            elif type == "list":
                if not isinstance(val, list):
                    return filename, f"Invalid metadata format: {val} should be a list."
            elif type == "bool":
...
def lowercase_filename(filepath):
    ...
    exempt_files = [(".".join(x)) for x in itertools.product(name, file_extensions)]
...
def valid_image_links(line, **kwargs):
    ...
    if match:
        ...
        if image and image.endswith("/"):
            ...
            image_path = str(kwargs.get("filename"))[
                :-8
            ]  # Remove "index.md" from the end of the path
            if not os.path.isfile(image_path + image):
                return (
                    kwargs.get("line_num"),
                    image,
                    "Image link points to nonexistent file.",
                )
...
def find_files(path=".", ignore_paths=[], extensions=["MD", "md"], recursive=False):
    ...
    if ignore_paths:
        ...
        ignore_paths_resolved = list(
            map(lambda path: str(Path(path).resolve()), ignore_paths)
        )
        # Map() returns an array like:
        # ['^/home/travis/build/sitebay/docs/ignored_dir1/*', '^/home/travis/build/sitebay/docs/ignored_dir2*']
        ignore_paths_as_regexes = list(
            map(lambda path: "^{}.*".format(path), ignore_paths_resolved)
        )
        ...
        list_of_files = list(
            filter(
                lambda path: not re.match(ignore_path_regex_or, str(path)),
                list_of_files,
            )
        )
...
class TestManager(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.files = find_files(
            path=input_dir, ignore_paths=ignore_paths, recursive=True
        )
    ...
    def _function_mapper(self, obj, **kwargs):
        ...
        if type(obj) is list:
            ...
            for idx in range(len(obj)):
                res += map(
                    methodcaller("__call__", obj[idx], line_num=idx, **kwargs),
                    _validate["line"],
                )
    ...
    def _run(self):
        for filepath in self.files:
            ...
            lines, front_matter = (
                readfile(filepath, section=s) for s in ("content", "metadata")
            )
