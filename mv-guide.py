...
def exit_script(custom_message):
    ...
    print(
        "The command will add aliases from your guides' and sections' old locations, \nand it will stage all changes when finished."
    )
    sys.exit(1)
...
def parse_aliases(alias_line):
    ...
    aliases_as_clean_strings = map(lambda x: x.strip(" '[]"), aliases_as_messy_strings)

    return set(
        map(
            lambda x: ("" if os.path.isabs(x) else "/")
            + os.path.normpath(x).lower()
            + "/",
            aliases_as_clean_strings,
        )
    )
    ...
    with open(markdown_file, "r+") as fd:
        ...
        for line in contents:
            if "aliases: " in line and frontmatter_begin:
                ...
                fd.write("aliases: " + serialized_alias_set({new_alias}) + "\n" + line)
                frontmatter_line_written = True
            elif "---" in line:
...
def path_contents(path):
    ...
    # ('docs/development/javascript', ['deploy-a-react-app-on-sitebay'], ['_index.md'])
    ...
    return list(
        map(lambda tail: os.path.join(contents[0], tail), contents[1] + contents[2])
    )
...
def move_under_new_parent_directory(content_dir, paths, new_parent_directory):
    ...
    for path in paths:
        try:
            ...
            print_error_and_continue(e, "Caught error when attempting to run 'git mv':")
        except sh.ErrorReturnCode:
            print_error_and_continue(
                e,
                "Caught error when attempting to clean up old directory.\n\
...
def rename(content_dir, old_path, new_path):
    sh.mkdir(new_path)
    move_under_new_parent_directory(content_dir, path_contents(old_path), new_path)
    try:
...
def move_guides(paths):
    ...
    if os.path.isfile(paths[-1]):
        exit_script(f"Path '{path}' ({os.path.abspath(path)}) cannot be a file.")

    # If the last argument is a directory that already exists
    elif os.path.isdir(paths[-1]):
        move_under_new_parent_directory(content_dir, paths[:-1], paths[-1])
    ...
    try:
        ...
        print_error_and_continue(e, "Caught error when attempting to stage changes:")
...
print(
    "This command does not update the titles of your guides, please remember to do so if appropriate."
)
