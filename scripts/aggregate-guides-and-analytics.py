    ...
    with open(aggregate_file, 'w+') as file:
      ...
      for guide in guides:
          ...
          for row in csv.reader(open(analytics_file, "r", encoding='utf-8'), delimiter=","):
              if len(row) < 3:
                  continue
              if row[0] == "www.sitebay.org" + guide.uri:
                  visitors = row[1]
                  page_views = row[2]
          ...
          for row in csv.reader(open(search_file, "r", encoding='utf-8'), delimiter=","):
              if len(row) < 5:
                  continue
              if row[0] == "https://www.sitebay.org" + guide.uri:
                  clicks = row[1]
                  impressions = row[2]
                  ctr = row[3]
                  position = row[4]
          ...
          writer.writerow(["=HYPERLINK(\"https://www.sitebay.org" + guide.uri + "\",\" " + guide.title + "\")", guide.published, guide.modified, guide.slug, guide.deprecated, guide.deprecated_link, visitors, page_views, clicks, impressions, ctr, position])
