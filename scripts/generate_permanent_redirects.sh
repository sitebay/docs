...
# our production Object Storage bucket, which is `sitebaydocs-latestrelease`, in
# the us-east cluster.
...
egrep -R '<!DOCTYPE html><html><head><title>(https:\/\/.*\/)<\/title><link rel="canonical" href="(https:\/\/.*\/)"\/><meta name="robots" content="noindex"><meta charset="utf-8" \/><meta http-equiv="refresh" content="0; url=(https:\/\/.*\/)" \/><\/head><\/html>' ../public | while read -r line ; do
    ...
    # Example: /docs/databases/mariadb/how-to-install-mariadb-on-linux-7/
    ...
    # Example: /guides/how-to-install-mariadb-on-linux-7/index.html
    redirect_from_path="$(echo $line | cut -d ':' -f 1 | cut -c 10-)"

    echo "Redirecting /docs$redirect_from_path to $redirect_target_path"
    s3cmd --config=$(pwd)/.s3cfg --access_key=$OBJ_ACCESS_KEY --secret_key=$OBJ_SECRET_KEY put --no-mime-magic --acl-public --no-preserve --add-header=x-amz-website-redirect-location:$redirect_target_path ../public$redirect_from_path s3://sitebaydocs$DEPLOY_SUFFIX/docs$redirect_from_path
done